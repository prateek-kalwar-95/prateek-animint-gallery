...
image_links <- sprintf("https://raw.githubusercontent.com/%s/main/Capture.png", dt$viz_owner_repo)
...
Source <- sprintf('<a href="%s">%s</a>', dt$source, dt$title)
...
Image_HTML <- sprintf(
  '<img src="%s" style="width:150px;height:100px;object-fit:cover;" onerror="this.onerror=null; this.src=\'https://via.placeholder.com/150x100?text=No+Image\';">',
  image_links
)
...
kable(full_data, format = "html", escape = FALSE) %>%
  kable_styling(
    full_width = TRUE, # Set to TRUE for full width or FALSE for auto width
    bootstrap_options = c("striped", "hover", "condensed"),
    font_size = 14,
    position = "center"
  ) %>%
  column_spec(1, width = "180px") %>% # Image column width
  column_spec(2, width = "200px") %>% # Visualization Link column width
  column_spec(3, width = "300px") %>% # Source column width
  column_spec(4, width = "150px") %>% # Date column width
  scroll_box(width = "100%", height = "600px") # Add scrollable box
```
...
* [This gallery repository](https://github.com/prateek-kalwar-95/prateek-gallery)